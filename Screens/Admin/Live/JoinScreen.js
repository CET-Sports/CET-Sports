...
export default function JoinScreen({ setScreen, screens, roomId }) {
  ...
  const joinCall1 = async id => {
    const roomRef = await db.collection('rooms').doc(id+0);
  ...
  const joinCall2 = async id => {
    const roomRef = await db.collection('rooms').doc(id+1);
    const roomSnapshot = await roomRef.get();
    ...
    <>
      ...
      <View style={styles.callButtons} >
        ...
        <View styles={styles.buttonContainer} >
          ...
          {localStream && <Button title='Click to join call' onPress={() => joinCall1(roomId)} disabled={!!remoteStream} />}
        </View>
        <View styles={styles.buttonContainer} >
          {!localStream && <Button title='Click to start stream' onPress={startLocalStream} />}
          {localStream && <Button title='Click to join call' onPress={() => joinCall2(roomId)} disabled={!!remoteStream} />}
        </View>
      </View>
